...
/**
 * Which gait the bird is in, and everything that follows from the answer. Section 6 of
 ...
 * is allowed to start. And it owns <b>body pitch</b>, which is a target per gait rather than a value
 ...
 * than in {@link BirdMoveControl} because it is a ground manoeuvre, and this is the layer that owns
...
public class BirdGaitControl {
    ...
    private void tickAirborne() {
        ...
        if (BirdGaitConfig.perchOnArrival && this.mob.getNavigation().isDone() && this.groundWithinReach()) {
            this.gait = Gait.DESCENDING;
        }
    }
    ...
    /**
     * Walking is the one gait with nothing to steer: vanilla's ground navigation and move control are
    ...
    private void tickLaunching() {
        ...
        float turned = Mth.approachDegrees(this.mob.getYRot(), this.launchYaw, BirdGaitConfig.launchTurnPerTick);
        this.mob.setYRot(turned);
        this.mob.yBodyRot = turned;
        if (Math.abs(Mth.degreesDifference(turned, this.launchYaw)) <= BirdGaitConfig.launchYawTolerance) {
            this.gait = Gait.AIRBORNE;
        }
    }

    /**
     * One rate-limited approach to whatever this gait wants the body pointed at, which is what makes
    ...
    private boolean groundWithinReach() {
        AABB swept = this.mob.getBoundingBox().expandTowards(0.0, -BirdGaitConfig.perchProbeDepth, 0.0);
        return !this.mob.level().noCollision(this.mob, swept);
    }

    /** Names the gait in the debug overlay. */
    ...
    public float getLaunchYaw() {
        return this.gait == Gait.LAUNCHING ? this.launchYaw : Float.NaN;
    }
}
